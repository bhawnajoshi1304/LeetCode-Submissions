...
class Solution {
public:
    vector<int> pre, post;  // Index mappings for quick lookup
    ...
    TreeNode* f(vector<int>& preorder, vector<int>& postorder, int preS, int postS, int postE) {
        ...
        int lrPostIdx=post[lRoot];   // Index of left root in postorder

        // Recursively build left and right subtrees
        root->left=f(preorder, postorder, preS+1, postS, lrPostIdx);
        root->right=f(preorder, postorder, pre[postorder[postE-1]], lrPostIdx+1, postE-1);
    ...
    TreeNode* constructFromPrePost(vector<int>& preorder, vector<int>& postorder) {
        ...
        for (int i=0; i<n; i++) {
            pre[preorder[i]]=i;  // Store preorder indices
            post[postorder[i]]=i;  // Store postorder indices
        }
